...
function OverviewPage({ profile, applications, savedJobs, matchCount, onNavigate }) {
  ...
  return (
    <div>
      ...
      <div className="mb-8 grid grid-cols-2 gap-5 lg:grid-cols-4">
        ...
        <StatCard label="AI Matches" value={matchCount} subtitle="Recommended now" icon={Zap} iconColor="#22c55e" />
      </div>
...
function SearchPage({ onApplied }) {
  ...
  const apply = async jobId => {
    ...
    try {
      ...
      toast.success(`Application submitted. Match score: ${result.data?.ai_match_score ?? '-'}%`)
      onApplied?.()
    } catch (error) {
...
function MatchesPage({ recommendations, onNavigate }) {
  return (
    <div>
      <PageHeader icon={Zap} title="AI Matches" subtitle="Smart recommendations ranked by skills, preferences, and role fit" />
      <div className="mb-6 grid grid-cols-1 gap-5 md:grid-cols-3">
        <StatCard label="Recommended Jobs" value={recommendations.length} subtitle="Active opportunities" icon={Zap} iconColor="#22c55e" />
        <StatCard label="Strong Matches" value={recommendations.filter(job => job.matchScore >= 60).length} subtitle="Ready to shortlist" icon={TrendingUp} />
        <StatCard label="Average Score" value={recommendations.length ? Math.round(recommendations.reduce((sum, job) => sum + job.matchScore, 0) / recommendations.length) : 0} subtitle="Across recommendations" icon={CheckCircle2} iconColor="#8b5cf6" />
      </div>
...
export default function SeekerDashboard() {
  ...
  const renderPage = () => {
    switch (activePage) {
      case 'overview':
        return <OverviewPage profile={profile} applications={applications} savedJobs={savedJobs} matchCount={recommendations.length} onNavigate={setActivePage} />
      case 'search':
        return <SearchPage onApplied={() => loadDashboardData(user)} />
      case 'guidance':
        return <GuidancePage profile={profile} />
      case 'profile':
      ...
      case 'saved':
        return <EmptyPlaceholder icon={Bookmark} title="Saved Jobs" description="Jobs you bookmark will appear here." action={<Button size="sm" onClick={() => setActivePage('search')}>Browse Jobs</Button>} />
      case 'matches':
        return <MatchesPage recommendations={recommendations} onNavigate={setActivePage} />
      default:
        return null
    }
  }
