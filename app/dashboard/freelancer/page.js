...
function contractProgress(contractId, milestonesByContract) {
...
function currencyTotal(rows, statuses = []) {
  return (rows || [])
    .filter(row => !statuses.length || statuses.includes(row.status))
    .reduce((sum, row) => sum + Number(row.amount || 0), 0)
}

function OverviewPage({ fp, contracts, milestonesByContract, escrowRows, disputes, onNavigate }) {
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                  ...
                  <div key={contract.id} className="border-b border-[rgba(0,207,253,0.06)] px-6 py-4 last:border-0">
                    <div className="mb-3 flex items-start justify-between gap-4">
                      <div>
                        ...
                        <p className="mt-0.5 text-xs text-gray-400">{contract.projects?.title || 'Project'} · {contract.client?.full_name || 'Client'}</p>
                      </div>
        ...
        <div className="flex flex-col gap-5">
          <div className="rounded-xl p-5 text-white" style={{ background: 'linear-gradient(135deg, #0e2233 0%, #1a3347 100%)' }}>
            ...
            <p className="mb-2 text-3xl font-bold">{formatBD(fp?.wallet_balance || 0)}</p>
            ...
            <div className="flex flex-col gap-3">
              {nextMilestones.slice(0, 3).map(item => (
                <div key={item.id} className="rounded-xl border border-[#00cffd]/10 p-4">
                  ...
                  <p className="text-xs text-gray-400">{item.contract.title}</p>
            ...
            <DCard key={project.id} className="cursor-pointer transition-all duration-200 hover:border-[rgba(0,207,253,0.3)] hover:shadow-md">
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 text-xs text-gray-400">{project.profiles?.full_name} · {timeAgo(project.created_at)}</p>
...
function ProjectsPage({ contracts, milestonesByContract, escrowRows, onNavigate }) {
  return (
    <div>
            ...
            <DCard key={contract.id}>
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{contract.projects?.title || 'Project'} · {contract.client?.full_name || 'Client'}</p>
                </div>
...
function ContractsPage({ contracts, milestonesByContract, escrowRows, onSubmitMilestone, actionMilestoneId }) {
  return (
    <div>
        ...
        {contracts.length > 0 ? contracts.map(contract => {
          ...
          return (
            <DCard key={contract.id} noPad>
              <DCardHeader title={contract.title} subtitle={`${contract.client?.full_name || 'Client'} · ${formatBD(contract.amount)} total`} action={<StatusBadge status={contract.status} />} />
...
function EarningsPage({ fp, escrowRows }) {
  const transactions = [...escrowRows].sort((a, b) => new Date(b.released_at || b.created_at) - new Date(a.released_at || a.created_at)).map(row => ({
    ...
    clientName: row.from_user?.full_name || 'Client',
  }))
...
function DisputesPage({ contracts, disputes, onRaiseDispute, raisingDispute }) {
          ...
          <DCard key={dispute.id}>
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div>
                ...
                <div className="mt-3 flex flex-wrap gap-4 text-xs text-gray-400"><span>Opened {timeAgo(dispute.created_at)}</span>{dispute.amount_disputed && <span>Amount: {formatBD(dispute.amount_disputed)}</span>}<span>{dispute.contracts?.title || 'Contract'}</span></div>
...
function ReviewsPage({ fp }) {
  const reviews = [
    ...
    { name: 'StartupHub BH', project: 'App Prototype', date: 'Jan 2026', rating: 5, comment: 'Outstanding Figma work and a smooth project handoff.' },
  ]
    ...
    <div>
      ...
      <div className="flex flex-col gap-4">
        {reviews.map((review, index) => (
          <DCard key={index}><div className="flex items-start gap-4"><Avatar name={review.name} size="md" /><div className="flex-1"><div className="mb-1 flex items-center justify-between"><p className="text-sm font-semibold text-gray-900 dark:text-white">{review.name}</p><span className="tracking-wide text-yellow-400">{'★'.repeat(review.rating)}</span></div><p className="mb-3 text-xs text-gray-400">{review.project} · {review.date}</p><p className="text-sm leading-relaxed text-gray-500 dark:text-gray-400">“{review.comment}”</p></div></div></DCard>
        ))}
      </div>
    </div>
  )
...
export default function FreelancerDashboard() {
  ...
  const loadDashboardData = useCallback(async currentUserId => {
    const [profileResult, fpResult, proposalsResult, contractsResult] = await Promise.all([
      supabase.from('profiles').select('*').eq('id', currentUserId).single(),
      supabase.from('freelancer_profiles').select('*').eq('id', currentUserId).single(),
      supabase.from('proposals').select('*, projects(id, title, budget_max, status, updated_at, profiles!projects_client_id_fkey(full_name))').eq('freelancer_id', currentUserId).order('created_at', { ascending: false }),
      supabase.from('contracts').select('*, projects(id, title, status, budget_max, updated_at), client:profiles!contracts_client_id_fkey(id, full_name)').eq('freelancer_id', currentUserId).order('created_at', { ascending: false }),
    ])
    const contractRows = contractsResult.data || []
    const contractIds = contractRows.map(item => item.id)
    const [milestonesResult, escrowResult, disputesResult] = contractIds.length ? await Promise.all([
      supabase.from('milestones').select('*').in('contract_id', contractIds).order('created_at', { ascending: true }),
      supabase.from('escrow').select('*, contracts(title), milestones(title), from_user:profiles!escrow_from_user_id_fkey(full_name), to_user:profiles!escrow_to_user_id_fkey(full_name)').in('contract_id', contractIds).order('created_at', { ascending: false }),
      supabase.from('disputes').select('*, contracts(id, title, client_id, freelancer_id)').in('contract_id', contractIds).order('created_at', { ascending: false }),
    ]) : [{ data: [] }, { data: [] }, { data: [] }]

    setProfile(profileResult.data || null)
    setFp(fpResult.data || null)
    setProposals(proposalsResult.data || [])
    setContracts(contractRows)
    setMilestones(milestonesResult.data || [])
    setEscrowRows(escrowResult.data || [])
  ...
  const pageTitles = { overview: 'Dashboard', browse: 'Browse Projects', proposals: 'My Proposals', projects: 'Active Projects', alerts: 'Project Alerts', earnings: 'Earnings & Escrow', contracts: 'Contracts', disputes: 'Disputes', portfolio: 'Portfolio', profile: 'My Profile', reviews: 'Reviews', messages: 'Messages', settings: 'Settings', billing: 'Billing' }
  ...
  const renderPage = () => {
    switch (activePage) {
      ...
      case 'profile': return <ProfileEditor profile={profile} roleProfile={fp} onSaved={() => loadDashboardData(user.id)} />
      ...
      case 'proposals': return <div><PageHeader icon={Send} title="My Proposals" subtitle="Track submitted proposals and accepted project entries" /><DCard noPad>{proposals.map(proposal => <ListRow key={proposal.id}><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-gray-900 dark:text-white">{proposal.projects?.title}</p><p className="text-xs text-gray-400">Bid: {formatBD(proposal.bid_amount)} · {proposal.delivery_days}d · {timeAgo(proposal.created_at)}</p></div><StatusBadge status={proposal.status} /></ListRow>)}{proposals.length === 0 && <EmptyPlaceholder icon={Send} title="No proposals yet" description="Submit proposals on open projects to get started." action={<Button size="sm" onClick={() => setActivePage('browse')}>Browse Projects</Button>} />}</DCard></div>
      case 'alerts': return <EmptyPlaceholder icon={Bell} title="Project Alerts" description="Saved searches and matching alerts can be expanded next." action={<Button size="sm">Set Up Alert</Button>} />
      default: return null
    }
  }

  return <DashboardLayout user={user} profile={profile} navItems={NAV} activePage={activePage} onNavigate={setActivePage} pageTitle={pageTitles[activePage] || 'Dashboard'} topbarRight={<Button size="sm" onClick={() => setActivePage('browse')}><Search className="h-4 w-4" /> Browse Projects</Button>}>{renderPage()}</DashboardLayout>
}
