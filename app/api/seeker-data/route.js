...
export async function GET(request) {
  try {
    ...
    const [profileResult, seekerProfileResult, applicationsResult, savedJobsResult, jobsResult] = await Promise.all([
      ...
      adminClient.from('applications').select('*, jobs(title)').eq('seeker_id', userId).order('created_at', { ascending: false }),
      adminClient.from('saved_jobs').select('*, jobs(*)').eq('seeker_id', userId),
      adminClient.from('jobs').select('*').eq('status', 'active').order('created_at', { ascending: false }).limit(30),
    ])
